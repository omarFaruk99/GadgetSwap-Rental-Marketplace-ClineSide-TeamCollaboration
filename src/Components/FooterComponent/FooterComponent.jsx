  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div
          className={`absolute -top-24 -right-24 w-52 h-52 rounded-full opacity-15 ${darkMode ? "bg-purple-600" : "bg-indigo-700"
            }`}
        ></div>
        <div
          className={`absolute top-1/2 -left-24 w-64 h-64 rounded-full opacity-15 ${darkMode ? "bg-blue-600" : "bg-blue-400"
            }`}
        ></div>
        <div
          className={`absolute -bottom-32 right-1/4 w-80 h-80 rounded-full opacity-15 ${darkMode ? "bg-cyan-600" : "bg-cyan-400"
            }`}
        ></div>
      </div>
      ...
      <div className="container mx-auto px-4 pt-20 pb-5 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 mb-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {footerLinks.map((section) => (
                <div key={section.id} className="space-y-4">
                  ...
                  {/* Desktop Header */}
                  <h3
                    className={`text-lg font-bold hidden lg:block ${darkMode ? "text-white" : "text-gray-900"
                      }`}
                  >
                    {section.title}
                  </h3>
                  ...
                  {/* Links */}
                  <ul
                    className={`space-y-3 ${activeSection === section.id || window.innerWidth >= 1024
                      ? "block"
                      : "hidden lg:block"
                      }`}
                  >
                    {section.links.map((link, linkIndex) => (
                      <li key={linkIndex}>
                        ...
                        >
                          <span
                            className={`mr-2 transition-colors ${darkMode
          ...
          <div className="lg:col-span-4 space-y-6">
            <div
              className={`p-6 rounded-xl backdrop-blur-md ${darkMode
            ...
            >
              ...
              <form onSubmit={handleSubscribe} className="space-y-4">
                ...
                {isSubscribed && (
                  <p
                    className={`text-center animate-pulse ${darkMode ? "text-green-400" : "text-green-600"
                      }`}
                  >
