  ...
  async (userEmail, { rejectWithValue }) => {
    try {
      const response = await fetch(
        `${BASE_URL}/gadgets/get_gadget_details_of_a_wishlist_array`,
        {
          method: "POST",
      ...
      if (data.status !== 200)
        throw new Error(
          data.message || "Failed to get wishlist gadget details!"
        );
      return data.data;
    } catch (error) {
...
export const addOrRemoveWishlistGadget = createAsyncThunk(
  ...
  async ({ userEmail, gadgetId }, { rejectWithValue, dispatch }) => {
    try {
      const response = await fetch(
        `${BASE_URL}/users/add_or_remove_a_gadget_id_to_or_from_wishlist`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
